...
class ProductSupplierInfo(models.Model):
    ...
    scope_ids = fields.Many2many('service.scope', string="Services", related='product_tmpl_id.scope_ids')
    pol_id = fields.Many2one(comodel_name='port.cites', string='POL', related='product_tmpl_id.pol_id')
    pod_id = fields.Many2one(comodel_name='port.cites', string='POD', related='product_tmpl_id.pod_id')
    transport_type_id = fields.Many2one('transport.type', string="Transport Type", related='product_tmpl_id.transport_type_id')
    shipment_scope_id = fields.Many2one('shipment.scop', string="Shipment Scope",
                                        domain=[('type', '=', 'sea')], related='product_tmpl_id.shipment_scope_id')
    package_type = fields.Many2one('package.type', string="Package Type", related='product_tmpl_id.package_type')
    container_type = fields.Many2one('container.type', string="Container Type", related='product_tmpl_id.container_type')
