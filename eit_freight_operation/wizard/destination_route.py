...
class DestinationRoute(models.Model):
    _name = "destination.route"

    transport_type_id = fields.Many2one('transport.type', string="Transport Type")
    port_id = fields.Many2one('port.cites', string="Transit Port", domain="[('type_id', '=', transport_type_id)]")
    port_id_origin = fields.Many2one('port.cites', string="Origin Port",
                                     domain="[('type_id', '=', transport_type_id)]")
    port_id_destination = fields.Many2one('port.cites', string="Destination Port",
                                          domain="[('type_id', '=', transport_type_id)]")
    pickup_address = fields.Text(string="Delivery Address")
    ...
    origin_services_ids = fields.One2many('origin.services', 'dest_route_id', string="Services")
    ...
    def create_destination_route(self):
        for record in self:
            record.task_id.deatination_route = [(4, record.id)]
